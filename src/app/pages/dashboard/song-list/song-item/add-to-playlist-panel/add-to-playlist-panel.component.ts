...
export class AddToPlaylistPanelComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.playlists = this.songService.allPlaylists
            .map((playlist: Playlist) => {
                let checked = false;
                for (let song of playlist.songs) {
                    if (song.id === this.song.id) checked = true;
                }
                return new PlaylistSmall(playlist.id, playlist.name, checked);
            })
            .filter(
                (playlist: PlaylistSmall) =>
                    playlist.name !== Constants.FAVOURITE_SONGS &&
                    playlist.name !== this.songService.currentPlaylistName
            );

        this.allPlaylistsSubscription = this.songService.allPlaylistsChanged.subscribe((allPlaylists: Playlist[]) => {
